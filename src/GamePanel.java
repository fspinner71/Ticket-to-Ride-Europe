...
public class GamePanel extends JPanel implements MouseListener {
    ...
    public static BufferedImage map, city, deck, ticket;
    public static BufferedImage[] cards, buttons;
    public static BufferedImage bottomBar, leftBar, rightBar, upArrow, downArrow, errorWindow, notebookPaper,
            textBoxLarge, textBoxSmall;
    ...
    private Button back, okButton, endTurnButton, confirm, arrowup, arrowdown, arrowup2, arrowdown2, acceptButton,
            declineButton;
    private int action, numberoflocomotivestheywanttouse, routebuyingcolor, extraCardsNeeded,
            startofgamewhenupickthetickets, endofgamestations, stationcounthing; // 0 = hasnt picjed, 1 = draw, 2 =
                                                                                 // route, 3 = station, 4 = ticket
    private Game game;
    ...
    private int selectedTrack, ticketdisplayindex = 0;
    ...
    public GamePanel(Frame f) {
        ...
        gameCards = new Button[6]; // like when you draw cards wtv it has includes the face down card ;; 0 =
                                   // facedown, 1-5 = cards

        gameCards[0] = new Button(1645, 115, cards[0].getWidth() / 5, cards[0].getHeight() / 5, deck); // deck button
        for (int c = 1; c < gameCards.length; c++) {
            gameCards[c] = new Button(1645, 115 + c * 135, cards[0].getWidth() / 5, cards[0].getHeight() / 5, deck); // face
                                                                                                                     // up
                                                                                                                     // cards
                                                                                                                     // button
                                                                                                                     // image
        ...
        actions[0] = new Button(1600, 300 - 10, buttons[0].getWidth() / 2, buttons[0].getHeight() / 2,
                buttons[Game.BLUE]);
        actions[1] = new Button(1600, 410 - 10, buttons[0].getWidth() / 2, buttons[0].getHeight() / 2,
                buttons[Game.YELLOW]);
        actions[2] = new Button(1600, 520 - 10, buttons[0].getWidth() / 2, buttons[0].getHeight() / 2,
                buttons[Game.PINK]);
        actions[3] = new Button(1600, 630 - 10, buttons[0].getWidth() / 2, buttons[0].getHeight() / 2,
                buttons[Game.ORANGE]);
        ...
        // confirm = new Button(1600, 820, buttons[0].getWidth() / 2,
        // buttons[0].getHeight() / 2, buttons[Game.GREEN]);
    ...
    public void paint(Graphics g) {
        ...
        if (game.errorPanel == true) {
            ...
            g.drawImage(bottomBar, getWidth() / 2 - bottomBar.getWidth() / 2, getHeight() - bottomBar.getHeight(),
            ...
            for (int i = 0; i < players.length; i++) {
                ...
                g.drawImage(Track.owns[i], -5, 260 * i, 160, 50, null);
            ...
            if (game.gameEnded && action != -1) { // game endded time to pick statons and stuff
                ...
                g2.drawString("Player " + (endofgamestations + 1) + " pick your route for your "
                        + stationsaplayerowns.get(stationcounthing).getName() + " station", 325, 822);
            ...
            if (game.gameEnded == false) {
                // draw player x
                g2.drawString("Stations", bottomBar.getWidth() * 5 / 9 + leftBar.getWidth() + 10,
                        getHeight() - bottomBar.getHeight() + 85);
                g2.drawString("Points", bottomBar.getWidth() * 6 / 9 + leftBar.getWidth() + 15,
                        getHeight() - bottomBar.getHeight() + 85);
                g2.drawString("Cars", bottomBar.getWidth() * 7 / 9 + leftBar.getWidth() + 20,
                        getHeight() - bottomBar.getHeight() + 85);
                g.drawImage(textBoxLarge, (bottomBar.getWidth()) * 5 / 9 + leftBar.getWidth(),
                        getHeight() - bottomBar.getHeight() + 100, 130, 150, null);
                g.drawImage(textBoxLarge, (bottomBar.getWidth()) * 6 / 9 + leftBar.getWidth(),
                        getHeight() - bottomBar.getHeight() + 100, 130, 150, null);
                g.drawImage(textBoxLarge, (bottomBar.getWidth()) * 7 / 9 + leftBar.getWidth(),
                        getHeight() - bottomBar.getHeight() + 100, 130, 150, null);

                g2.drawString("" + game.players[game.turn].getNumStations(),
                        bottomBar.getWidth() * 5 / 9 + leftBar.getWidth() + 50,
                        getHeight() - bottomBar.getHeight() + 180);
                g2.drawString("" + game.players[game.turn].getPoints(),
                        bottomBar.getWidth() * 6 / 9 + leftBar.getWidth() + 50,
                        getHeight() - bottomBar.getHeight() + 180);
                g2.drawString("" + game.players[game.turn].getNumTrains(),
                        bottomBar.getWidth() * 7 / 9 + leftBar.getWidth() + 50,
                        getHeight() - bottomBar.getHeight() + 180);
                g2.setFont(bigFont);
                ...
                for (int c = 0; c < game.getPlayers()[game.turn].getTrainCards().length; c++) {
                    for (int i = 0; i < game.getPlayers()[game.turn].getTrainCards()[c]; i++) { // 325 900
                        g.drawImage(rotatecounterclockwise(cards[c]), 325 + tempimagegapthing * imagegap, 830,
                                rotatecounterclockwise(cards[c]).getWidth() / 5,
                                rotatecounterclockwise(cards[c]).getHeight() / 5, null);
                        tempimagegapthing++;
                    }
                }
            ...
            for (City a : game.getCities()) {
                if (a.hasStation()) {
                    g.drawImage(stations[0], GamePanel.MAP_X + a.getXCoord() - 25 / 2 - 7,
                            GamePanel.MAP_Y + a.getYCoord() - 25 / 2 - 12, stations[0].getWidth() / 50,
                            stations[0].getHeight() / 50, null);
                }
            }
            ...
            if (action == -1) { // -1 is if the turn ended and itll juust show the end turn button
                ...
                g2.drawString("END TURN", 1650, 515);
            ...
            if (action == 0) { // 0 is like the action screen and the start of their turn
                ...
                buyingTunnel = false;
                threetickets = null;
                pickingtickets[0] = false;
                pickingtickets[1] = false;
                pickingtickets[2] = false;
            ...
            if (action == 2) { // 2 is if they decide to buy a route
                ...
                g2.drawString(arrayforchoosingroutecolor[routebuyingcolor],
                        1715 - (arrayforchoosingroutecolor[routebuyingcolor].length() - 3) * 6, 585);
            ...
            if (action == 3) { // they wnana buy a station
                ...
                g2.drawString(arrayforchoosingroutecolor[routebuyingcolor],
                        1675 - (arrayforchoosingroutecolor[routebuyingcolor].length() - 5) * 8, 585);
                g2.drawString("Station", 1690, 97);
                if (theywannaplacestationon != null) {
                    g2.setFont(font);
                    g2.drawString(theywannaplacestationon.getName(),
                            1688 - (theywannaplacestationon.getName().length() - 7) * 8, 474);
                }
            }
            if (action == 4) { // pick tickets
                ...
                threetickets = game.drawTicket();
                for (int c = 0; c < threetickets.length; c++) {
                    threetickets[c].setX(1630);
                    threetickets[c].setY(205 + c * 170);
                    threetickets[c].paint(g);
                }
                for (int c = 0; c < pickingtickets.length; c++) {
                    if (pickingtickets[c] == true) {
                        g2.drawString("whoa", threetickets[c].getX(), threetickets[c].getY());
                    }
                }
            ...
            if (buyingTunnel) {
                ...
                for (int c = 0; c < 3; c++) {
                    g2.drawImage(rotatecounterclockwise(cards[threecards[c]]),
                            725 + errorWindow.getWidth() / 32 + c * 100, 300 + errorWindow.getHeight() / 12,
                            rotatecounterclockwise(cards[c]).getWidth() / 5,
                            rotatecounterclockwise(cards[c]).getHeight() / 5, null);
                }
                g2.drawString("Number of extra cards needed:" + extraCardsNeeded, 725 + errorWindow.getWidth() / 32,
                        340);
                ...
                System.out.println(
                        "how many they have of buying color: " + game.players[game.turn].numOfColor(routebuyingcolor));
                // game.buyTunnel(currentlyBuying, routebuyingcolor, extraCardsNeeded,
    ...
    public void mousePressed(MouseEvent e) {
        System.out.println(game.getPlayers()[0].findRoute("VENEZIA", null, "BUDAPEST"));
        if (e.getButton() == MouseEvent.BUTTON1) {
            ...
            if (game.errorPanel == true) {
                ...
                if (action == -2) {
                    for (int c = 0; c < fourtickets.length; c++) {
                        if (x > fourtickets[c].getX() && y > fourtickets[c].getY() && x < fourtickets[c].getX() + 250
                                && y < fourtickets[c].getY() + 165) {
                ...
                if (action == -1) {
                    ...
                    ArrayList<Ticket> currentplayertickets = game.getPlayers()[game.turn].getTickets();
                    if (currentplayertickets.isEmpty() == false) {
                        if (x > currentplayertickets.get(ticketdisplayindex).getX()
                                && y > currentplayertickets.get(ticketdisplayindex).getY()
                                && x < currentplayertickets.get(ticketdisplayindex).getX() + 250
                                && y < currentplayertickets.get(ticketdisplayindex).getY() + 165) {
                    ...
                    for (int c = 0; c < threetickets.length; c++) {
                        if (x > threetickets[c].getX() && y > threetickets[c].getY() && x < threetickets[c].getX() + 250
                                && y < threetickets[c].getY() + 165) {
